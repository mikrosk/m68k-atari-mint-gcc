...
(define_expand "<trigos><mode>2"
  [(set (match_operand:FP 0 "nonimmediate_operand" "")
        (unspec:FP [(match_operand:FP 1 "register_operand" "")] TRIGOS)
   )]
  "TARGET_68881 && flag_unsafe_math_optimizations"
  "")
  
(define_insn "<trigos><mode>2_68881"
  [(set (match_operand:FP 0 "nonimmediate_operand" "=f")
	(unspec:FP
	  [(match_operand:FP 1 "general_operand" "f<FP:dreg>m")] TRIGOS))]
  "TARGET_68881 && flag_unsafe_math_optimizations"
{
  if (FP_REG_P (operands[1]))
    return "f<tricod>%.x %1,%0";
  else
    return "f<tricod>%.<FP:prec> %1,%0";
})
